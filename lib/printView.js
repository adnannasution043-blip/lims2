...
function testTypeLabel(name, row, item) {
  if (name === 'Hardness Test') {
    return row.hardness_spot ? `Hardness Test, Jumlah Spot : ${esc(row.hardness_spot)}` : esc(name);
  }
  ...
  const fl = row.charpy_fl ? esc(row.charpy_fl) : '......';
  const fl2 = row.charpy_fl2 ? esc(row.charpy_fl2) : '......';
  const opt = row.charpy_optional ? esc(row.charpy_optional) : '......';
  const optLabel = esc(row.charpy_optional_label || 'Opsional/Lainnya');
  return `Charpy Impact Test, T&deg; : ${t}<br>WM : ${wm}&nbsp;&nbsp;BM : ${bm}&nbsp;&nbsp;HAZ : ${haz}` +
    `<br>FL : ${fl}&nbsp;&nbsp;FL+2 : ${fl2}&nbsp;&nbsp;${optLabel} : ${opt}`;
}

function couponTbody(row, idx) {
  const otherTests = (row.other_tests || []).filter(ot => ot.test_name);
  const totalRows = TEST_TYPES.length + otherTests.length;

  const fixedRows = TEST_TYPES.map((name, i) => {
    const item = (row.test_items || []).find(ti => ti.test_name === name) || {};
    const firstCell = i === 0
      ? `<td class="no-col" rowspan="${totalRows}">${idx + 1}.</td>
         <td class="desk-col" rowspan="${totalRows}">${deskripsiFieldsHtml(row)}</td>`
      : '';
    return `<tr>
      ${firstCell}
      <td class="cek-col">${item.checked ? '&#10003;' : ''}</td>
      <td class="jenis-col">${testTypeLabel(name, row, item)}</td>
      <td class="jml-col">${item.checked ? esc(item.qty) : ''}</td>
      <td class="metode-col">${item.checked ? esc(item.method) : ''}</td>
    </tr>`;
  }).join('');

  const otherRows = otherTests.map(ot => `<tr>
    <td class="cek-col">&#10003;</td>
    <td class="jenis-col">${esc(ot.test_name)}</td>
    <td class="jml-col">${esc(ot.qty)}</td>
    <td class="metode-col">${esc(ot.method)}</td>
  </tr>`).join('');

  return `<tbody class="coupon-block">${fixedRows}${otherRows}</tbody>`;
}
