...
const TASK_STAGES = [
  ...
  { key: 'preparation', label: 'Preparation', hint: 'Marking, cutting, machining specimen', kind: 'derived', picColumn: 'machining_pic', dateLabel: '' },
  ...
  app.get('/api/work-order-progress', async (req, res) => {
    try {
      ...
      const { rows: wos } = await pool.query(
        `SELECT wo.id, wo.test_request_id, wo.testing_date, ${picColumns},
...
module.exports = {
  ...
  evalStage, computeStatuses, inspectionStatus, taskStatus, sanitizeTask, buildInfo, testRowsOf, couponRowsOf
};
