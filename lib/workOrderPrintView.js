...
function testTypeLabel(ti, row) {
  if (ti.test_name === 'Hardness Test') {
    return row.hardness_spot ? `Hardness Test, Jumlah Spot : ${esc(row.hardness_spot)}` : esc(ti.test_name);
  }
...
function woBlock(row, idx, sampleMarking) {
  ...
  const combined = [...checkedItems, ...otherTests];
  const items = combined.length ? combined : [{ test_name: '-', qty: '', method: '' }];
  ...
  <div class="sheet">
    ...
    <div class="approval-wrap">
      ...
      <table class="approval-table">
        ...
        <tr>
          <td class="approval-sig">${wo.prepared_by_signature ? `<img class="sig-img" src="${esc(wo.prepared_by_signature)}" alt="">` : ''}</td>
          <td class="approval-sig">${wo.checked_by_signature ? `<img class="sig-img" src="${esc(wo.checked_by_signature)}" alt="">` : ''}</td>
          <td class="approval-sig">${wo.approved_by_signature ? `<img class="sig-img" src="${esc(wo.approved_by_signature)}" alt="">` : ''}</td>
        </tr>
        <tr>
